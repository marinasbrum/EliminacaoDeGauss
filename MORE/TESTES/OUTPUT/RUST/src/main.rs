...
fn main() {
    ...
    let elapsed_time = etstop.duration_since(etstart).unwrap();
    let cpu_time_used = etstop2.duration_since(etstart2);
    ...
    let mut file = File::create("output.txt").expect("Error opening file.");

    // Escrever as informações no arquivo
    writeln!(file, "=================================================").expect("Error writing to file.");
    writeln!(file, "Matrix dimension N = {}", n).expect("Error writing to file.");
    writeln!(file, "Elapsed time = {:.3} ms.", elapsed_time.as_secs_f64() * 1000.0)
        .expect("Error writing to file.");
    writeln!(file, "(CPU times are accurate to the nearest {:.3} ms)", 1.0 / 1000.0)
        .expect("Error writing to file.");
    writeln!(file, "My total CPU time = {:.3} ms.", cpu_time_used.as_secs_f64() * 1000.0)
        .expect("Error writing to file.");
    writeln!(file, "=================================================").expect("Error writing to file.");
}
